...
template<typename T,typename A> vector_rep<T,A>::vector_rep(const A& a,int n){
  std::cout<<"Constructor vector_rep\n";
  try{
...
template<typename T,typename A> vector_rep<T,A> & vector_rep<T,A>::operator=(vector_rep<T,A>&& other) {
  std::cout<<"Move assignment operator for vector_rep\n";
  if (this != &other) {
    if (elem) {
...
/*Default constructor*/
template<typename T,typename A> vector<T,A>::vector(): r(A{},0){std::cout<<"Constructor vector\n";}
/*Constructor*/
template<typename T,typename A> vector<T,A>::vector(int s,T value): r(A{}, s){
  std::cout<<"Constructor vector(int s,T value)\n";
  if(r.elem!=nullptr)
...
template<typename T,typename A> vector<T,A>::~vector(){std::cout<<"Destructor vector()\n";}
